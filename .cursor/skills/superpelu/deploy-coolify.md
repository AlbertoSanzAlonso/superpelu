...
Ante caída de OpenWA/Chromium: Superpelu **watchdog cada 60s** + reintentos + stop→start **solo ante zombi** (timeout/ProtocolError ×3, cooldown 45 min). Manual: `POST https://TU-DOMINIO/api/admin/whatsapp/reconnect` o Restart OpenWA en Coolify. No meter scripts `.sh` en el volumen; Custom Start Command de locks va **inline** en Coolify.
