  ...
  return (
    <div ref={rootRef} className="relative">
      <button
        ...
        aria-label={
          badge > 0
            ? `Notificaciones de citas, ${badge} sin leer`
      ...
      {open && (
        <div
          className="absolute right-0 top-[calc(100%+0.5rem)] z-50 w-[min(100vw-1.5rem,20rem)] border border-gold/30 bg-cream shadow-[0_16px_48px_-16px_rgba(30,30,30,0.4)]"
          role="menu"
        >
          <div className="border-b border-gold/15 px-3 py-2">
            <p className={`${typography.label} text-gold`}>Citas nuevas</p>
          </div>
          {inbox.length === 0 ? (
            <p className="px-3 py-4 text-center text-xs text-charcoal-muted">
              No hay citas nuevas desde la última vez.
            </p>
          ) : (
